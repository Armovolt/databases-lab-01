...
**Посилання на репозиторій GitHub:** https://github.com/Armovolt/databases-lab-01
...
**Найповільніший запит (Корельований підзапит із завдання 3.1):**
...
<img width="1538" height="853" alt="image" src="https://github.com/user-attachments/assets/d8010c43-02b7-43fb-9dfd-a7999e03d739" />

**Запропоновані оптимізації:**
1. Переписати з використанням CTE/Віконних функцій: Замість багаторазового виклику підзапиту для кожного рядка (O(N²)), агрегацію краще виконати один раз через AVG(unit_price) OVER(PARTITION BY category_id).
2. Створення композитного індексу: Додати індекс на поля (category_id, unit_price) в таблиці products, що дозволить оптимізатору швидше знаходити середнє значення.
3. Оновлення статистики: Регулярно виконувати ANALYZE products, щоб планувальник PostgreSQL обирав оптимальний алгоритм з'єднання (Hash Join замість Nested Loop).
...
**Обґрунтування:** Цей покриваючий (covering) індекс критично важливий для пришвидшення запитів, які агрегують або фільтрують товари за ціною в межах певних категорій (як у нашому корельованому підзапиті чи віконних функціях).
...
**Обґрунтування:** Оптимізує роботу віконних функцій типу LAG()/LEAD(), які здійснюють розділення (PARTITION BY customer_id) і сортування (ORDER BY order_date). Індекс дозволяє системі пропустити етап важкого сортування Sort Method: quicksort.


### Порівняльний аналіз

#### Ефективність різних підходів
**Завдання:** Знайти топ-2 найдорожчих товарів у кожній категорії
...
SELECT * FROM RankedProducts WHERE rn <= 2;
```
...
**Час виконання (за даними EXPLAIN ANALYZE):**
- Віконні функції: ~0.150 ms (виконується в 1 прохід завдяки сортуванню)
- Корельований підзапит: ~1.250 ms (багатократне циклічне сканування)

**Висновок:** Підхід із використанням віконної функції ROW_NUMBER() є суттєво (майже в 10 разів) ефективнішим за корельований підзапит. Віконні функції сканують таблицю лише один раз і виконують сортування у пам'яті, тоді як корельований підзапит примушує СУБД перераховувати COUNT(*) для кожного окремого товару, що на великих обсягах даних призведе до катастрофічного падіння продуктивності.
...
**Обгрунтування:** Було виконано абсолютно всі завдання лабораторної роботи на найвищому, 3-му рівні складності. Набуто практичного досвіду роботи із різними видами JOIN'ів, навмисно досліджено логіку різновидів агрегацій (GROUP BY та HAVING). Успішно імплементовано аналітичні інструменти: віконні функції для ранжування й пошуку дельт у датах, CTE, рекурсивні запити для деревоподібної структури (керівник-підлеглий). Крім того, проведено глибокий перформанс-аналіз (EXPLAIN ANALYZE) і на практиці доведено перевагу віконних функцій над застарілими корельованими підзапитами, а також обґрунтовано створення нових композитних індексів. Робота відповідає всім критеріям для оцінки "відмінно".
